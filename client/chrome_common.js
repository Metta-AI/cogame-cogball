...
//
// Cogball keeps every mechanism (plate rendering, feed rows, beat markers, the
// momentum curve, the spoilers switch, the end card) and changes only the FIELD
// MAPPING: two sides named azure/crimson, goals in place of lives, and the
// goal/drop beats in place of steal/return/capture.
//
...
window.ChromeCommon = function (ctx) {
  ...
  // the RIGHT — Azure on the left of the clock, Crimson on the right, matching
  // the pitch (Azure defends -x). Colors must stay in sync with the CSS
  // .azure / .crimson team utility classes.
  // Cogball seats exactly two sides. Azure rides the shipped BLUE rig, Crimson
  // the RED one, so the chrome tints match the board art with no recolouring.
  var TEAM_ORDER = ['azure', 'crimson'];
  var TEAM_COLOR = { azure: BLUE, crimson: RED };
  function teamCol(team) { return TEAM_COLOR[team] || null; }
  function activeTeams(s) {
    var keys = (s && s.teams) ? Object.keys(s.teams) : [];
    if (!keys.length) return ['azure', 'crimson'];
  ...
  function teamName(s, team, fallback) {
    ...
    // one policy — show it — or, on a shared side, exactly two: headline BOTH
  ...
  // ctf's per-team handicap badge and its perk-icon strip are arena rules
  // cogball does not have (see docs/RULES.md 'Out of scope'): both sides
  // play identical robots on an identical pitch.
  ...
  function renderClock(s) {
    ...
    if (s.ph === 'lobby') {
      ...
      // The coaching turn is the unit a spectator follows: one directive per
      // turn, forty turns a match.
      capEl.textContent = (s.turns > 0)
        ? ('turn ' + Math.min(s.turns, (s.turn || 0) + 1) + '/' + s.turns)
        : 'Time left';
  ...
  function otherTeam(t) {
    return t === 'azure' ? 'crimson' : t === 'crimson' ? 'azure' : '';
  }
  function ingestBeats(s) {
    ...
    for (var i = 0; i < beatTimeline.length; i++) {
      ...
      if (b.k === 'goal') markBeat(b.t, 'goal', b.team);
      else if (b.k === 'drop') markBeat(b.t, 'drop', '');
  ...
  // ---- momentum graph (GOAL DIFFERENCE over the WHOLE timeline) -----------
  // A step line + difference shading on the SAME 0→maxTicks x-axis as the seek
  // track, so a point's x IS its tick. Positive = Azure leads (azure shading
  // above the level midline), negative = Crimson leads (below). Samples are
  ...
  // The server ships the full-match GOAL change-point series ONCE (on the
  ...
  function ingestLeadSeries(s) {
    ...
    if (Array.isArray(s.lead)) {
      // Legacy shape: [[tick, azureGoals - crimsonGoals], …] — a two-side diff.
      fullLeadSeries = {
        teams: ['azure', 'crimson'],
        isDiff: true,
      ...
      // Team-keyed shape: {teams: [name…], pts: [[tick, goals…], …]}.
      fullLeadSeries = {
        teams: s.lead.teams || ['azure', 'crimson'],
        isDiff: false,
  ...
  function recordMomentum(s) {
    ...
    if (momentumSeen[s.t]) return;  // deterministic replay: a tick's score is fixed
    ...
    momentumSamples.push({
      ...
      vals: teams.map(function (team) {
        return (tr[team] && tr[team].goals) || 0;
      })
    });
    momentumDirty = true;
  }
  function renderMomentum() {
    ...
    var norm = fullLeadSeries ||
      { teams: momentumTeams || ['azure', 'crimson'], isDiff: false, pts: momentumSamples };
    ...
    if (norm.teams.length === 2) {
      ...
      // level midline (equal goals), shaded in the leading team's color above/below.
      ...
      // shading split honest. Floor at 1 so a goalless match never divides by
      // zero.
      ...
      // More than two sides has no scalar lead, so draw one team-coloured goal
      // line per side on a shared 0..peak scale. Cogball always seats two, so
      // this branch is a generality the shared chrome keeps.
      var peak = 1;
      pts.forEach(function (m) {
  ...
  return {
    ...
    markBeat: markBeat, renderBeatMarkers: renderBeatMarkers,
    ingestBeats: ingestBeats, setVerdict: setVerdict,
